...
public class rarest_build {
	query_graph query_graph;
	Origin_graph origin_graph;
	List<Integer> result_seq;
	int max_rarest;
	public rarest_build(query_graph query_graph,Origin_graph origin_graph) throws Exception {
		this.query_graph=query_graph;
		this.origin_graph=origin_graph;
		this.result_seq=new ArrayList<Integer>();
		this.max_rarest=Integer.MIN_VALUE;
	}
	 ...
	 * @return 返回最大值
	 */
	public int get_maxdia(List<Integer> seq){
	...
	public void set_result() {
			...
			*/
			...
			if(mem_rusult.size() == query_graph.get_seqLength()) { 	// 如果mem_rusult是一个完整序列
				if(this.result_seq.size()==0) {		    //判断result序列是否为空
					...
					this.set_max_rarest(this.get_maxdia(mem_rusult));		 //设置最大直径
					//System.out.println(this.get_max_rarest());
				}else {
					int new_max=this.get_maxdia(mem_rusult); 		//得到mem_rusult的最大直径
					if(new_max<this.get_max_rarest()) {				//	判断当前最大直径是否为最小
						this.clear_result_seq();					//最小，则清空result序列
						for(int nodeID:mem_rusult)	{				//将mem_rusult中节点依次加入到result中
							this.result_seq.add(nodeID);			//
							//System.out.print(nodeID+" ");
						}
						this.set_max_rarest(new_max);				//将最大直径设为新值
						//System.out.println(this.get_max_rarest());
					}
				}
			}
	 ...
	 public void set_max_rarest(int new_max) {
		 this.max_rarest=new_max;
		
	}
	 public void clear_result_seq() {
		 this.result_seq.clear();
	 }
	 public int  get_max_rarest() {
		 return this.max_rarest;
