...
public class Origin_graph implements OriginGraphInterface {
	...
	public int addEdge(int node1,int node2,int weight){
		...
		if(flag==1) {
			System.out.println(node1+"与"+node2+"The edge has exist!!");
             ...
             //System.out.println("("+node1+","+node2+","+weight+")"+"插入边成功！");
	 ...
	 * 添加m条边
	 */
	public void add_some_edges(int m) {
		  int count = 0;
		  Random r=new Random();
		  while(count<m) {
			  int node1=r.nextInt(this.graph.length);        //用随机数作为要添加的边的顶点；
			  int node2=r.nextInt(this.graph.length);
			  int weight=r.nextInt(4);
			  if(addEdge(node1,node2,weight)==0) {
				  count++;
				  //System.out.println("插入第"+count+"条边成功"); 
			  }
		  }
		  System.out.println("共插入"+count+"条边成功");  
	}
	
	/*
	 * 删除一条边
	 */
	public int deleteEdge(int node1,int node2) {
		Edge user=this.graph[node1];
		int flag=0;
		Edge pre=null;
		while(user!=null) {
		...
		if(flag==1) {
			...
			user=this.graph[node2];
			while(user!=null) {
				if(user.getadjvex()==node1) {
					if(pre==null) {                                  //当第一条边为要删除的边时；
						this.graph[node2]=user.next;
			...
			//System.out.println("("+node1+","+node2+") this edge does not exist");
		}
		return flag;
	}
	 ...
	 * 删除m条边
	 */
	public void delete_some_edegs(int m) {
		int count=0;
		int node1;
		int node2;
		Random r=new Random();
		while(count<m) {
			node1=r.nextInt(this.graph.length);
			node2=r.nextInt(this.graph.length);
			if(deleteEdge(node1,node2)==1) {
				count++;
				//System.out.println("删除第"+count+"条边"+"("+node1+","+node2+","+")成功！");
			}
		}
		System.out.println("共删除"+count+"条边成功");
	}
	@Override
	/*
