...
public class BloomFilter<E> implements Serializable {
    ...
    /**
     * Custom by Special purpose
     * support create based old BloomFilter
     * @author LQ
     */
    public BloomFilter(BloomFilter<E> bloom) {
        //如果传入的旧bloom过滤器为空，则按照配置新创建一个
        if(bloom == null) {
            bloom = new BloomFilter<E>(Config.BLOOM_FALSE_POSTIVE_RATE, Config.BLOOM_EXPERCT_SIZE);
        }
        this.bitset = (BitSet) bloom.getBitSet().clone();
        this.bitSetSize = bloom.getBitSetSize();
        this.bitsPerElement = bloom.getBitsPerElement();
        this.expectedNumberOfFilterElements = bloom.getExpectedNumberOfFilterElements();
        this.numberOfAddedElements = bloom.getNumberOfAddedElements();
        this.k = bloom.getK();
    }
